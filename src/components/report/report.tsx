  ...
  return (
    <div className="screen:px-8">
      <div className="print:hidden max-w-lg border border-orange-600 rounded-lg p-3 mb-8 text-center text-lg mx-auto">
        This is printable version of the OWASP ASVS report. You can print it to
        PDF for export.
        <div className="flex gap-3 place-content-between px-6 py-2">
          <a
            href="/"
            className="flex gap-2 justify-center w-fit items-center border rounded-md px-2 py-1 no-underline	text-black"
          >
            <ArrowLeftCircle /> Go back{" "}
          </a>
          <button
            onClick={() => window.print()}
            className="flex gap-2 justify-center  items-center border rounded-md px-2 py-1"
          >
            {" "}
            Print <Printer />
          </button>
      ...
      <div
        id="report"
        className="screen:border screen:border-grey-100 screen:p-3 screen:rounded-lg"
      >
              ...
              <tbody>
                <tr>
                  ...
                  <td
                    className={cn(
                      "text-right",
                      getColorForResult(getTotalScore(results), results.length),
                    )}
                  >
            ...
            <div>
              <table>
                <thead>
                  <tr>
                    <td>Category</td>
                    <td>Checked controls</td>
                  </tr>
                </thead>
                <tbody>
                  {getFlatResultPerCategory(data).map((d, i) => (
                    <tr>
                      <td>{catResults[i].name}</td>
                      <td className={getColorForResult(d.checked, d.total)}>
                        {d.total
                          ? `${d.checked} / ${d.total} (${getPercent(
                              d.checked,
                              d.total,
                            )} %)`
                          : "no controls"}
                    ...
                    <div className="grid grid-cols-3 text-center divide-y ">
                      {cat.items
                        ...
                        .map((c) => (
                          <>
                            <span
                              className={getColorForResult(c.checked, c.total)}
                            >
                              {c.shortcode}
                            </span>
                            <span
                              className={cn(
                                "col-span-2 py-1",
                                getColorForResult(c.checked, c.total),
                              )}
                            >
                              {c.checked} / {c.total} (
          ...
          <tbody>
            {results.map((r) => (
              <tr key={r.Shortcode}>
                ...
                <td className="w-12">
                  {r.NA ? (
                    <p className="font-bold text-center w-full">N/A</p>
                  ) : r.checked ? (
