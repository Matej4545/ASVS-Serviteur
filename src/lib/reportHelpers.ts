...
export function getResultForLevel(data: ASVSAudit) {
  ...
  return controlsForLevel.flatMap((i1) =>
    i1.Items.flatMap((i2) =>
      i2.Items.filter(
        (i3) => !findAuditRes(data.results, i3.Shortcode)!.NA,
      ).map((i3) => ({
        ...i3,
  ...
  const enriched = controlsForLevel.map((i1) => ({
    ...
    items: i1.Items.map((i2) => ({
      ...
      checked: i2.Items.filter(
        ...
        (acc, i3) =>
          findAuditRes(data.results, i3.Shortcode)!.checked ? (acc += 1) : acc,
        0,
      ),
      total: i2.Items.filter(
        (i3) => !findAuditRes(data.results, i3.Shortcode)!.NA,
      ).length,
    })),
  }));
...
export function getFlatResultPerCategory(result: ASVSAudit) {
  return getResultForCategories(result).map((r) =>
    r.items.reduce(
      (acc, item) => {
        return {
